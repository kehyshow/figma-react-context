...
export function TicketsCard() {
  ...
  const data = useMemo(() => sortTickets(filterTickets(tickets, { status: statusFilter }, search), sortOption), [statusFilter, search, sortOption]);
      ...
      <>
        <MenuItem onClick={() => changeStatusFilter("")}>
          <PopoverContent className="mx-none">
            ...
            <Badge type="circle" className="ml-auto" variant={variants.PRIMARY}>{tickets.length}</Badge>
          </PopoverContent>
        </MenuItem>
        <MenuItem onClick={() => changeStatusFilter("Open")}>
          <PopoverContent className="mx-none">
            <CardHeaderText>Open</CardHeaderText>
            <Badge type="circle" className="ml-auto" variant={variants.OPEN}>{filterTickets(tickets, { status: "open" }).length}</Badge>
          </PopoverContent>
        </MenuItem>
        <MenuItem onClick={() => changeStatusFilter("Feedback")}>
          <PopoverContent className="mx-none">
            <CardHeaderText>Feedback</CardHeaderText>
            <Badge type="circle" className="ml-auto" variant={variants.FEEDBACK}>{filterTickets(tickets, { status: "feedback" }).length}</Badge>
          </PopoverContent>
        </MenuItem>
        <MenuItem onClick={() => changeStatusFilter("Resolved")}>
          <PopoverContent className="mx-none">
            <CardHeaderText>Resolved</CardHeaderText>
            <Badge type="circle" className="ml-auto" variant={variants.RESOLVED}>{filterTickets(tickets, { status: "resolved" }).length}</Badge>
          </PopoverContent>
        </MenuItem>
      </>
    );
    ...
    <Card>
      <CardHeader>
        ...
        <ActionGroup>
          ...
          >
            <PopoverContent>
              <CardHeaderText>{statusFilter === "" ? "All Tickets" : statusFilter}</CardHeaderText>
              <Badge type="circle" className="badge" variant={statusFilter === "" ? "primary" : statusFilter.toLowerCase()}>{filterTickets(tickets, { status: statusFilter }).length}</Badge>
              <PopoverIcon>
                <GoChevronDown />
              </PopoverIcon>
            </PopoverContent>
      ...
      <CardBody>
        {tickets.length === 0 ?
          <EmptyContent
            ...
            content={"Your support tickets or feature requests will appear here."}
          />
          :
          (
            data.length ?
              ...
              <EmptyContent
                ...
                content={"Please adjust your search term and try again."}
              />
          )
        }
      </CardBody>
