...
import { StyledTable, StyledTH, StyledTD, TicketTitle, TypographyText, ForumTitle, Avatar, AuthorContainer, StaffContainer, ReplyCountContainer } from "./styles";
...
export function TicketTable({ data, isLoading, handleSort, sortOption }: TableProps) {
  return (
    <>
      {
        isLoading ?
              ...
              {!isLoading && data && data.map((item) => (
                <tr key={item.id}>
                  ...
                  <StyledTD maxWidth={67}>
                    <Badge type="rounded" variant={item.status}>{item.status}</Badge>
                  </StyledTD>
                  ...
                  <StyledTD removePaddingRight={true}>
                    <AuthorContainer>
                      <Avatar>
                        <img src={item.latest_post_author.avatar_url} alt="avatar" />
                      </Avatar>
                      <TypographyText>
                        Last by {item.latest_post_author.display_name}
                      </TypographyText>
                    </AuthorContainer>
                  </StyledTD>
                  <StyledTD removePaddingRight={true}>
                  ...
                  <StyledTD removePaddingRight={true}>
                    {
                      ...
                      <Badge variant={variants.PRIMARY} type="rounded" className="staff-badge">
                        <img alt="staff badge" src={StaffBadge} />
                        Staff
                      </Badge>
                    }
                  </StyledTD>
